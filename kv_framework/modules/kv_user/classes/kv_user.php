...
require_once ( KVUSER_ROOT."/classes/kv_user_db.php");
...
class KvUser extends KvUserDB {
    ...
    //
        ...
        if($pw == $db_pw) {
            ...
            $now = time();
            if (($now - $db_last_access) > (KvUser::DEFAULT_TIMEOUT)) {
                // timeout reached
    ...
    // login
    //
        $this->errMsg("[KvUser::login] try : $id, $pw");
    ...
    //
        ...
        // if cookie exists
            list($id,$hash) = explode(',',$_COOKIE[KvUser::DEFAULT_COOKIE_NAME]);
            // get password
